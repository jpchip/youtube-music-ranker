...
function getUserStats(db: Database) {
...
router.get("/users", async (_req, res) => {
  try {
    ...
    res.json({
      summary: { totalUsers, totalBattles, totalSongs },
...
router.get("/users/:id/stats", async (req, res) => {
  try {
    ...
    const recentMatchesResult = db.exec(`
      ...
      FROM matches m
      JOIN songs s1 ON m.song1_id = s1.video_id
      JOIN songs s2 ON m.song2_id = s2.video_id
